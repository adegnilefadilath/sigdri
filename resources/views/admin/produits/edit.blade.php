...
@section('sous_titre', $produit->designation)
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="text-sm font-bold text-gray-800">Modifier le produit</h3>
        ...
        {{-- Badge statut si inactif --}}
        @if (! $produit->actif)
        <div class="mx-6 mt-4 flex items-center gap-2 px-4 py-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl text-sm">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                      d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
            </svg>
            Ce produit est actuellement <strong>inactif</strong>. La modification est possible mais il n'apparaîtra pas dans les formulaires de déclaration.
        </div>
        @endif

        <form method="POST" action="{{ route('admin.produits.update', $produit->id) }}" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="unite_industrielle_id"
                    ...
                    @foreach ($unites as $u)
                    <option value="{{ $u->id }}"
                            {{ old('unite_industrielle_id', $produit->unite_industrielle_id) == $u->id ? 'selected' : '' }}>
                        {{ $u->denomination }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="unite_mesure"
                           value="{{ old('unite_mesure', $produit->unite_mesure) }}"
                           class="w-full px-3 py-2.5 text-sm rounded-xl border
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-3 py-2.5 text-sm rounded-xl border
                                 ...
                                 focus:outline-none focus:ring-2 focus:ring-[#1a237e] focus:border-transparent transition-colors resize-none">{{ old('description', $produit->description) }}</textarea>
            </div>
